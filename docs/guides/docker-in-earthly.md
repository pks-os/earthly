...
```Dockerfile
hello:
    FROM earthly/dind:alpine-3.20-docker-26.1.3-r2
...
`FROM earthly/dind:alpine-3.20-docker-26.1.3-r2` inherits from an Earthly-supported docker-in-docker (dind) image. This is recommended, because `WITH DOCKER` requires all the Docker binaries (not just the client) to be present in the build environment.
...
It's recommended to use the `earthly/dind:alpine-3.20-docker-26.1.3-r2` image for running docker-in-docker. See the best-practices' section on using [with docker](../guides/best-practices.md#use-earthly-dind) for more details.
